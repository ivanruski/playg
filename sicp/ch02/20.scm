...
(define (same-parity n . ns)
  (if (odd? n)
      (filter odd? (cons n ns))
      (filter even? (cons n ns))))
...
;; without filter
(define (same-parity n . ns)
  (define (parity-iter ns parity-test)
    (if (null? ns)
        '()
        (let ((num (car ns))
              (rest (cdr ns)))
          (if (parity-test num)
              (cons num (parity-iter rest parity-test))
              (parity-iter rest parity-test)))))
  (if (odd? n)
      (cons n (parity-iter ns odd?))
      (cons n (parity-iter ns even?))))
