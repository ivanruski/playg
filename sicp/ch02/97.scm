...
;;   this factor.
;;
;; Exercise 2.97. a. Implement this algorithm as a procedure reduce-terms that
;; takes two term lists n and d as arguments and returns a list nn, dd, which
;; are n and d reduced to lowest terms via the algorithm given above. Also write
;; a procedure reduce-poly, analogous to add-poly, that checks to see if the two
...
;;     (list (/ n g) (/ d g))))
;;
;; and define reduce as a generic operation that calls apply-generic to dispatch
;; to either reduce-poly (for polynomial arguments) or reduce-integers (for
;; scheme-number arguments). You can now easily make the rational-arithmetic
;; package reduce fractions to lowest terms by having make-rat call reduce
