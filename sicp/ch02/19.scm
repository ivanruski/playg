;; Exercise 2.19. Consider the change-counting program of section 1.2.2. It would be
;; nice to be able to easily change the currency used by the program, so that we
;; could compute the number of ways to change a British pound, for example. As the
;; program is written, the knowledge of the currency is distributed partly into the
...
;; The order of the list does not affect the answer, it only affects the
;; generated representations during the calculation.
;; For example:
;; (cc 5 (list 1 2)) will find that (1, 2, 2) is a valid representation,
;; whereas (cc 5 (list 2 1)) will find taht (2, 2, 1) is a valid representation.
;; We are trying to find all possible ways to represent a sum with a given coins
;; and the order in which we are using them doesn't matter.
