...
;; (x, y) in the region and false for points not in the region. For example, the
;; region contained within a circle of radius 3 centered at (5, 7) is described
;; by the predicate that tests whether (x - 5)^2 + (y - 7)^ 2 <= 3^2. To
;; estimate the area of the region described by such a predicate, begin by
...
;; above. The desired integral is the area of that portion of the rectangle that
;; lies in the region. We can estimate the integral by picking, at random,
;; points (x,y) that lie in the rectangle, and testing P(x, y) for each point to
